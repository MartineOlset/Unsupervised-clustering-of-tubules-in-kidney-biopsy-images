...
class roi_dataset(Dataset):
    def __init__(self, img_csv, trnsfrms_val):
        super().__init__()
...
def get_feature(path, trnsfrms_val): 
    all_features = []
    img_csv=pd.read_csv(path)
    test_datat=roi_dataset(img_csv, trnsfrms_val)
    database_loader = torch.utils.data.DataLoader(test_datat, batch_size=1, shuffle=False)

    model = ResNet.resnet50(num_classes=128,mlp=False, two_branch=False, normlinear=True)
    pretext_model = torch.load(r'./best_ckpt.pth', map_location='cpu')
